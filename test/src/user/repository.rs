...
use crate::user::model::{Account, User};
...
pub async fn exists_by_email(pool: &PgPool, email: &str) -> Result<bool> {
    let result: (bool,) = query_as(r#"SELECT EXISTS(SELECT 1 FROM "user" WHERE email = $1)"#)
        .bind(email)
        .fetch_one(pool)
        .await?;
    Ok(result.0)
...
pub async fn update_password(pool: &PgPool, user_id: Uuid, password_hash: &str) -> Result<User> {
    let result = query_as(
        r#"
        UPDATE "user"
