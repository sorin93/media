/*
  TODO: add a secret when calling the REST API so no one else can send direct request to create posts with fake embedding vector, image descriptions etc.
  ...
  try {
    const res = await fetch(`https://api.sorin.cc${path}`, {
      ...
      headers: {
          ...
          // TODO: add secret here, and verify it in POST /media handler
      },
