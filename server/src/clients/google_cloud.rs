...
pub async fn get_embedding(
    ...
    // 1. Check Cache
    let cache = text::find_by_text(&state.pool, &text).await?;
    ...
    let embedding_vec = body["embedding"]["values"]
        .as_array()
        .ok_or_else(|| Error::Internal("Could not find 'embedding.values' in Vertex AI response".into()))?;
    ...
    if embedding.len() != 3072 {
        return Err(Error::Internal(format!("Expected embedding of dimension 3072, but got {}", embedding.len())));
    }

    // Convert and save to database
    let cache = text::insert(&state.pool, text, embedding.into()).await?;
