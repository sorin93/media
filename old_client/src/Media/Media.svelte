...
{#snippet player()}
  ...
  {#if result.index > 0}
    ...
    <a class="overlay-prev" href="/m/{result.media[result.index - 1].media_id}" data-replace use:link></a>
  {/if}

  <!-- Next -->
  {#if result.index < result.media.length - 1}
    <!-- svelte-ignore a11y_consider_explicit_label -->
    <a class="overlay-next" href="/m/{result.media[result.index + 1].media_id}" data-replace use:link></a>
  {/if}
  ...
  {#if !store.isMobile}
    <div class="actions">
      ...
      <!-- Prev -->
      {#if result.index > 0}
        <a href="/m/{result.media[result.index - 1].media_id}" data-replace use:link>
          <Svg
            aria-hidden="true"
            class="pointer"
            fill="var(--fill)"
